...
class ApiService {
  ...
  static void _handleError(dynamic error, StackTrace? stackTrace) {
    if (kDebugMode) {
      debugPrintStack(label: 'ApiService Error: $error', stackTrace: stackTrace);
    }
  }
  ...
  Future<dynamic> getWalletBalance(String address) async {
    try {
      final response = await http.get(Uri.parse('$_baseUrl/crypto/balance/$address'))
          .timeout(const Duration(seconds: 30));
      if (response.statusCode == 200) return jsonDecode(response.body);
  ...
  Future<dynamic> transferCrypto(String fromAddress, String toAddress, double amount, String currency) async {
    try {
      final response = await http.post(
